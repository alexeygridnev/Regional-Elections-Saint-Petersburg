...
for num_tik_row in range(len(listingfin)):
    filecsv=open("/zakso_fptp/zakso_fptp_tab"+str(num_tik_row+1)+".csv", encoding="utf-8", mode="w")
...
for n in range(len(codebooklist)):
    file=open("/zakso_fptp/zakso_fptp_codebook"+str(n+1)+'.txt', encoding="utf-8", mode="w")
    for p in range(18):
